...
public interface QuizMemberRepository extends JpaRepository<QuizMember, Long> {
    ...
    @Query("SELECT COUNT(qm) > 0 FROM QuizMember qm WHERE qm.member.id = :memerbId AND year(qm.solvedAt) = :year AND month(qm.solvedAt) = :month")
    Boolean existsByMemberAndDate(@Param("memberId") UUID memberId, @Param("year") int year, @Param("month") int month);
}
